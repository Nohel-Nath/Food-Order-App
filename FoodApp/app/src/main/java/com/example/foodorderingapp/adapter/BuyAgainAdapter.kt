...
class BuyAgainAdapter(
    ...
    //private val buyAgainResName:ArrayList<String>,
    ...
    inner class BuyAgainViewHolder(private val binding: BuyAgainItemBinding) :
        ...
        fun bind(foodName: String, foodPrice: String, foodImage: String) {
            binding.tvFoodItemName.text = foodName
            //binding.tvResName.text = resName
            binding.tvFoodPriceCart.text = foodPrice
            val uriString = foodImage
            val uri = Uri.parse(uriString)
            Glide.with(requireContext).load(uri).into(binding.ivFoodImage)
            //binding.ivFoodImage.setImageResource(foodImage)
        }
    }
